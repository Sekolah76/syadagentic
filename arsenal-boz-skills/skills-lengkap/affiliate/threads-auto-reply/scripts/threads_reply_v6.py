#!/usr/bin/env python3
"""Threads Auto-Reply v6.0 — COMBO FIX
Auto-reads links from database, uses CDP, handles all edge cases.
Run: cd /Users/user/.hermes/hermes-agent && venv/bin/python3 scripts/threads_reply_v6.py
"""
...
def cdp_eval(ws, expr, timeout=30):
    """Evaluate JS and return value. Uses IIFE pattern."""
...
def parse_database():
    """Parse affiliate-link-database.md to get UNUSED links."""
    links = []
    try:
        with open(DATABASE_PATH, 'r') as f:
            content = f.read()
        
        # Find table rows with UNUSED links
...
def search_posts(ws, keyword):
    """Search for posts with given keyword."""
...
def reply_to_post(ws, post_url, comment_text, link_url, product_name):
    """Reply to a specific post. Returns: 'success', 'pending', 'failed', 'restricted', 'hard_blocked', 'already_replied'"""
    ...
    # Find reply button (Comment/Balas/Reply + numbered) — use JS .click()
...
def main():
    log("=== Threads Auto-Reply v6.0 — COMBO FIX ===")
    
    # Parse database for UNUSED links
    ...
    for keyword in KEYWORDS:
        ...
        # Find unused link for this keyword's category
        category = "skincare"
        ...
        # Find first unused link in category
        link_info = None
