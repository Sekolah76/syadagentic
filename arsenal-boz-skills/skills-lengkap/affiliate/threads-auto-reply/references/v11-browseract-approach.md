...
**Solution:** Camoufox `chrome_local_102130715962900495` (real Chrome, BOZ's profile) with **manual one-time login** to jagonya_shopee. Real Chrome fingerprint = trusted = reply visible cross-account. Verified working 2026-06-30 even DURING active shadowban from 28 Jun Wardah strike.
...
# Then BOZ logs in manually in the opened Chrome tab.
...
- BOZ logs out from Threads on another device
- Cookie `sessionid` invalidated by Threads
- Camoufox browser process killed externally (pkill chromium)

Recovery: re-open `--headed`, BOZ logs in manually, session resumes.
...
def ensure_session():
    ...
    if SESSION_NAME not in out:
        # Reopen — but this comes up WITHOUT cookies, BOZ must login again
...
If `logged_in=False`, the script exits with instruction to BOZ to login manually. Cron tick still consumes a slot but doesn't waste minutes attempting reply against logged-out session.
...
3. **Strike events:** if BOZ gets a content-removal notification on jagonya_shopee, pause cron 48-72h for natural account-reputation recovery. Camoufox path is more resilient than Playwright but not invincible to repeat strikes.
