...
> **Hermes install (BOZ):** full upstream tree under `web-dev/hallmark/`.  
> Pipeline: `hallmark` → BUILD → `website-security-hardening` → DEPLOY (`vercel-single-file-site` when single-file).  
> Compact operational subset (BOZ defaults dark/crypto/streaming/local-biz): `anti-ai-slop-design`.  
> Upstream: https://github.com/Nutlope/hallmark · demo https://www.usehallmark.com · MIT.  
> **Hermes ops (reinstall, dual-stack routing, domain pre-brief, tweet fetch):** load [`references/hermes-boz.md`](references/hermes-boz.md).
