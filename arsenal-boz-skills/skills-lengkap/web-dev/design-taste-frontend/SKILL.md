...
* **`VISUAL_DENSITY: 4`** - 1 = Art BOZlery / Airy, 10 = Cockpit / Packed Data
...
**BOZ Mobile-First Override:** start every page at `320px` with one-column content and `48px` touch targets. Verify `320/375/414` in a browser before adding any desktop layout. Base utilities/CSS are mobile; use `min-width` only for additive layouts. Never rely on hover for an essential action, never let CTA/nav labels wrap, and redesign wide tables to mobile cards.
...
* **1-3 (Art BOZlery):** Lots of white space. Huge section gaps (`py-32` to `py-48`). Expensive, clean.
...
### BOZleries & Media
* **Dome BOZlery** - 3D panoramic gallery.
