...
description: Complete reference for 25 web2 bug classes with root causes, detection patterns, bypass tables, and real paid examples. Covers IDOR, auth bypass, XSS, SSRF (11 IP bypass techniques), SQLi, business logic, race conditions, OAuth/OIDC (PKCE, state, alg confusion, OIDC discovery-document misconfig, Spring x-application-context leak, .env/404 differential, Kong gateway version disclosure, actuator index footprint), file upload (10 bypasses), GraphQL, LLM/AI (ASI01-ASI10 + MCP/RAG/vector poisoning), API misconfig (mass assignment, JWT attacks, prototype pollution, CORS), ATO taxonomy, SSTI, subdomain takeover, cloud/infra, HTTP smuggling, cache poisoning, MFA bypass (7 patterns), SAML attacks, error disclosure/debug endpoints, CSS injection, LFI->RCE (filter chain RCE, log poisoning, polyglot .user.ini), insecure deserialization (PHP phar, Java ysoserial, Python pickle, Node node-serialize). Use when hunting a specific vuln class or studying what makes bugs pay.
---
...
## 25. OAUTH/OIDC DISCOVERY-DOCUMENT MISCONFIGURATION
> The `/.well-known/openid-configuration` document *advertises* unsafe defaults. Reading it is recon — no auth needed — and any dangerous value there is a finding regardless of whether the actual attack succeeds. See `references/oauth-oidc-recon-patterns.md` for the full probe matrix, JWT alg-none builder, and field-by-field danger taxonomy.

**Discovery (no auth):**
```bash
curl -sL https://TARGET/.well-known/openid-configuration | python3 -m json.tool
curl -sL https://TARGET/jwk | python3 -m json.tool
```

**Field-by-field danger taxonomy:**

| Field | Dangerous value | Why |
|---|---|---|
| `id_token_signing_alg_values_supported` | includes `"none"` | Server may accept JWTs with `alg: none` → ATO |
| `token_endpoint_auth_methods_supported` | includes `"none"` | `/token` accepts requests without client credentials |
| `subject_types_supported` | `"pairwise"` without per-client audience check | Cross-client user tracking / token confusion |
| `registration_endpoint` | reachable | Attacker can register a malicious client |
| `introspection_endpoint` | present | Cheap token-validity oracle |
| `scopes_supported` | write scopes (`all.write`, `purchase`, `crypto.withdraw`) | Documents API surface |
| `code_challenge_methods_supported` | missing `S256` | PKCE downgrade → code interception |

**JWT `none` attack vector** (per RFC 7518 §3.1, `alg: none` MUST NOT be used unless explicitly understood):
```
header  = {"alg":"none","typ":"JWT","kid":"<from JWKS>"}
payload = {"sub":"victim_id","iss":"<from OIDC>","exp":<future>}
token   = base64url(header) + "." + base64url(payload) + "."
```
If the server honors the advertised `none`, the verifier passes the token → ATO any user, no interaction.

**Companion patterns (same recon, frequently co-occur):**

- **Spring Boot `x-application-context: <app>:<profile>`** — every response carries the deployed Spring profile. `application:stage,errors` on a production URL = the server is serving **staging infrastructure** (weaker controls, test data, sample credentials). Same header on `/actuator/health` confirms Spring Cloud.
- **`.env` vs 404 differential** — dot-prefixed paths (`/.env`, `/.env.bak`, `/.env.production`, `/.git/HEAD`) return `400 "Invalid request"`, other unknown paths return `403 "Invalid resource"`. The split reveals what the server actively tries to parse. Try `.env.local`, `.env.dist`, `.env.sample`, `.env.old`.
- **Old Kong gateway + endpoint enumeration** — `Server: kong/0.9.7` (2018) + every `/payments/*` returns `401 "Kindly include a Base64 encoded key"` + `code: K001` + a `reference: <uuid>` log-correlation handle. The 401 (not 404) confirms routes exist.
- **Spring Boot Actuator index disclosure** — `GET /actuator` returns `{"_links":{"health":...,"health-path":...}}` documenting the actuator footprint even when only `/health` is exposed. Latent risk: future misconfig leaks `/env`, `/heapdump`, `/mappings`.

**Triage:**
```
OIDC "none" + server accepts forged token with victim sub     Critical (ATO)
OIDC "none" + server rejects at runtime                      High (config disclosure)
x-application-context: <app>:stage on production URL         High (deployment disclosure)
.env/403 differential + actual env file leak                 Critical (env vars leak)
.env/403 differential alone (no file present)                Low/Info
Kong/0.x server header + admin API exposed                   Critical (RCE via Kong CVE)
Actuator only /health exposed                                Informational
Actuator /env, /heapdump, /mappings exposed                  Critical (creds leak)
```
