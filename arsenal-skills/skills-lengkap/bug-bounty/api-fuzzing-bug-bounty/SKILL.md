...
## TEE-Protected API Breaking (ECIES + EIP-712)

For DEX/bridge APIs encrypting requests to Nitro/SGX enclaves, see `references/tees-ecies-perps-dex.md`.
...
## Pre-Auth Endpoint Fingerprinting (Error-Code Differential)

Compare error codes across endpoints to find ones that skip auth checks:

| Response pattern | Meaning | Action |
|---|---|---|
| `12100 / "not logged in"` | Auth gate fires first | Properly gated |
| `100002 / param error / timeout` | **Param validation BEFORE auth** | Priority target — find correct params |
| `404 / empty` | Wrong path | Move on |
| `401 / 403` | HTTP-layer auth gate | Properly gated |

Hit every endpoint with garbage params. Endpoints returning param-validation errors (not auth errors) process requests before checking identity. Brute-force correct param shape from JS bundle analysis.

**Real example (Tencent Docs):** `auth_exp/set` and `auth_getconf` returned `100002` while siblings `auth_info/get` and `set_user` returned `12100`. The 100002 endpoints process pre-auth.

## JS Bundle API Mining

1. `curl -sk <target> | grep -oE 'src="[^"]+\.js"'` → download bundles
2. `grep -oE '"[^"]*(/api/|/cgi-bin/|/v[0-9]/|/cgi-go/)[^"]*"'` → endpoint paths
3. Search endpoint string ±500 chars → param names and shape
4. `grep -oE '(xsrf|csrf|TOK|token).{0,100}'` → CSRF token source
5. Test param name variants: `padId`, `localPadId`, `pad_id`, `docId`
6. `grep -oE '(client_id|appid).{0,50}[0-9]{6,}'` → OAuth config

Minified JS preserves paths and param names verbatim. Calling function reveals exact shape (FormData vs JSON, GET vs POST).

### Named endpoint constants (high-value mining)

Minified bundles often define a single object of named route constants, e.g.
`users_polymarket_account:"/users/polymarket/account"`. Grepping this object
reveals the FULL auth-gated surface in one shot — endpoints plain path-guessing
never finds. This is how the whole Polymarket scope surfaced on Manic.trade:

```bash
# dump the whole route-constant object
grep -oE 'users_[a-z_]+:"/[^"]*"' chunk.js | sort -u
# or any prefix
grep -oE '[a-z_]+_auth[a-z_]*:"/[^"]*"' chunk.js | sort -u
```

Then probe each discovered path for auth status (401 vs 405 vs 200). A `405`
(needs POST) vs `401` (missing auth) split tells you which are method-gated vs
token-gated. Cross-check against the base URLs found in the same bundle
(`grep -oE 'https://[a-z0-9.-]*' *.js | sort -u`) — `/api/*` paths may live on a
DIFFERENT host than the one you're probing.

### Auth-bypass matrix (when endpoints return 401)

When all sensitive endpoints return `401 Missing authorization`, run this
matrix before assuming a token is required — cheap and sometimes hits:

- Header spoof: `X-Forwarded-For: 127.0.0.1`, `X-Original-URL`, `X-Rewrite-URL`,
  `X-Custom-IP-Authorization`, `X-Real-IP`, `X-Auth-Token`, `X-Api-Key`,
  `X-Access-Token`
- Token tricks: `Authorization: Bearer null`, `Bearer ` (empty), `Basic YWRtaW46YWRtaW4=`
- Method/path: `X-HTTP-Method-Override`, trailing slash, `//` double slash,
  `/api/./x`, `%2e`, case change, `;` semicolon, `?x=1` query

If ALL return the same `401 Missing authorization header`, auth is enforced
server-side (likely a middleware) and you need a real token — pivot to the
login/wallet flow. Don't keep fuzzing; the bypass isn't there. (Manic.trade:
every technique returned identical 401 — Turnkey-custodial auth, no bypass.)

## Batch Discipline (Context Management)

- Max 3-5 probes per terminal call
- Print only DIFF results (suppress baseline)
- `head -c 150` on bodies, never dump full HTML/JS
- One batch = one hypothesis
- After 3-4 batches with no new signal → STOP, reassess strategy
- End session with negative results table: `| Probe | Result | Severity |`

## Pitfalls

- **Don't brute-force endlessly.** After 3 batches with identical responses, the approach is wrong. Switch strategy (JS analysis, different param names, different content-type).
- **`retcode:0` with empty result ≠ success.** Check nested response codes (e.g., `result.code:1007` = user not login despite top-level `retcode:0`).
- **WAF fingerprinting:** Note which payload patterns trigger WAF (e.g., 501 responses) and avoid them in subsequent probes.
- **Session tokens are random per session.** Extract fresh xsrf/TOK from Set-Cookie each batch; never reuse across sessions.
