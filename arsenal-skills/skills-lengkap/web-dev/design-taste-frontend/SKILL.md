...
**SYADAGENTIC Mobile-First Override:** start every page at `320px` with one-column content and `48px` touch targets. Verify `320/375/414` in a browser before adding any desktop layout. Base utilities/CSS are mobile; use `min-width` only for additive layouts. Never rely on hover for an essential action, never let CTA/nav labels wrap, and redesign wide tables to mobile cards.
