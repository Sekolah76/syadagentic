...
> **Hermes install (SYADAGENTIC):** full upstream tree under `web-dev/hallmark/`.  
> Pipeline: `hallmark` → BUILD → `website-security-hardening` → DEPLOY (`vercel-single-file-site` when single-file).  
> Compact operational subset (SYADAGENTIC defaults dark/crypto/streaming/local-biz): `anti-ai-slop-design`.  
> Upstream: https://github.com/Nutlope/hallmark · demo https://www.usehallmark.com · MIT.  
> **Hermes ops (reinstall, dual-stack routing, domain pre-brief, tweet fetch):** load [`references/hermes-guide.md`](references/hermes-guide.md).
...
## SYADAGENTIC Mobile-First Override
