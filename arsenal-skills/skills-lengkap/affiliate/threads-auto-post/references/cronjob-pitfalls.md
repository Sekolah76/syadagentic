...
- Fix: Set `model: "SYADAGENTIC"` and `provider: "9router"` explicitly
...
cronjob(action='create', 
  ...
  prompt="...",
  schedule="30 7,11,19 * * *",
  model={"model": "SYADAGENTIC", "provider": "9router"},
  skills=["skill-name"]
)
```
...
```
cronjob(action='list')
→ Check each job has model: "SYADAGENTIC" and provider: "9router" set
→ If null → update with cronjob(action='update', job_id=..., model={"model": "SYADAGENTIC", "provider": "9router"})
```
