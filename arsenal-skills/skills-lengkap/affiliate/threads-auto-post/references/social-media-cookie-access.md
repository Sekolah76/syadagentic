...
### Twitter/X ⚠️ COOKIES EXTRACTABLE, POSTING REQUIRES MANUAL LOGIN
- Cookies: `auth_token`, `twid`, `ct0` indicate logged-in
- Domain: `.x.com`
- Handle: @MyBiniGua
- **Cookie extraction:** browser_cookie3 works → saves to `/Users/user/twitter_cookies.json`
- **BUT posting requires:** Manual login in browser tool session (Twitter blocks headless browser bot detection)
- **Status:** API tokens expired. To post: open Twitter login page → user manually logs in → extract cookies for verification
...
- Wallet addresses (OWL, SYADAGENTIC)
